...
from models import Pure_Bert, ComparisonModel
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='./', help='Directory to store intermedia data.')
    ...
    # Model parameters
    parser.add_argument('--w2v_dir', type=str, default='./GoogleNews-vectors-negative300.bin',
                        help='Directory storing glove embeddings')
    parser.add_argument('--glove_dir', type=str, default='../HuggingFaceH4/',
                        help='Directory storing glove embeddings')
    ...
    parser.add_argument('--embedding_type', type=str, default='bert', choices=['glove', 'bert', 'w2v'])
    parser.add_argument('--model_type', type=str, default='cnn', choices=['cnn', 'lstm', 'fc'])

    parser.add_argument('--embedding_dim', type=int, default=768, help='Hidden size [glove:300, bert:768, w2v:300]')
    ...
    parser.add_argument("--num_train_epochs", default=30.0, type=float, help="Total number of training epochs.")
...
def main():
    ...
    if args.dataset_name == 'hotel':
        train_dataloader = DataLoader(HotelDataset(args, True),
                                      batch_size=args.train_batch_size,
                                      shuffle=True,
                                      collate_fn=collate_fn)
        test_dataloader = DataLoader(HotelDataset(args, False),
                                     batch_size=args.train_batch_size,
                                     shuffle=False,
                                     collate_fn=collate_fn)
    else: #'amazon'
        train_dataloader = DataLoader(AmazonDataset(args, True),
                                      batch_size=args.train_batch_size,
                                      shuffle=True,
                                      collate_fn=collate_fn)
        test_dataloader = DataLoader(AmazonDataset(args, False),
                                     batch_size=args.train_batch_size,
                                     shuffle=False,
                                     collate_fn=collate_fn)
    ...
    model = ComparisonModel(args,
                            which_model=args.model_type,
                            which_embeding=args.embedding_type,
                            embedding_dim=args.embedding_dim)  # embed_dim = glove:300, bert:768, w2v:?
